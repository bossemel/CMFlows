...
class Test_Copula_PDF(unittest.TestCase):
    def __init__(self, *args, **kwargs):
        ...
        self.copula = 'clayton'
        self.copula_distr_1 = datasets.distributions.Copula_Distr(self.copula, self.theta, obs=self.obs)
        self.copula_distr_1.sampler(obs=self.obs)
        self.samples_1 = self.copula_distr_1.xx

    def test_one_copula_same_sample(self):
        X_in_p = self.copula_distr_1.pdf(self.samples_1)
        X_in_q = self.copula_distr_1.pdf(self.samples_1)
    ...
    def test_different_theta(self):
        ...
        samples_2 = copula_distr_2.xx

        X_in_p = self.copula_distr_1.pdf(self.samples_1)
        X_in_q = copula_distr_2.pdf(self.samples_1)
        Y_in_p = self.copula_distr_1.pdf(samples_2)
        Y_in_q = copula_distr_2.pdf(samples_2)
