...
class Test_JSD(unittest.TestCase):
    ...
    def test_one_pdf_same_samples(self):
        mv_gaussian_1 = scipy.stats.multivariate_normal(mean=[0, 0], cov=[[1., 0],
                                                                          [0, 1.]])
        samples_1 = mv_gaussian_1.rvs(self.obs)
        X_in_p = mv_gaussian_1.pdf(samples_1)
    ...
    def test_one_pdf_different_samples(self):
        mv_gaussian_1 = scipy.stats.multivariate_normal(mean=[0, 0], cov=[[1., 0],
                                                                          [0, 1.]])
        samples_1 = mv_gaussian_1.rvs(self.obs)
        samples_2 = mv_gaussian_1.rvs(self.obs)

        X_in_p = mv_gaussian_1.pdf(samples_1)
        X_in_q = mv_gaussian_1.pdf(samples_1)
        Y_in_p = mv_gaussian_1.pdf(samples_2)
        Y_in_q = mv_gaussian_1.pdf(samples_2)
    ...
    def test_different_cov(self):
        ...
        X_in_p = mv_gaussian_1.pdf(samples_1)
        X_in_q = mv_gaussian_2.pdf(samples_1)
        Y_in_p = mv_gaussian_1.pdf(samples_2)
